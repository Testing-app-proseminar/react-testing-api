...
function EditCardPage(props) {
    ...
    useEffect(() => {
        ...
        .then((response) => {
            ...
            setLink(card.link);
            setConsumeTime(card.consumeTime);
        })
    ...
    const handleFormSubmit = (e) => {
        ...
        axios
        .put(`${import.meta.env.VITE_API_URL}/cards/${cardId}/edit`, requestBody, {headers: {Authorization: `Bearer ${storedToken}`}})
        .then((response)=> {
    ...
    const deleteCard = () => {
        axios
        .delete(`${import.meta.env.VITE_API_URL}/cards/${cardId}`)
        .then(() => {
            navigate("/cards")
        })
        .catch((error) => {console.log(error)})
